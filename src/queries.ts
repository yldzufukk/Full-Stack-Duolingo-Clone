...
export const getUnits = cache(async () =>  {
  ...
  const normalizedData = data.map((unit) => {
    const lessonWithCompetedStatus = unit.lessons.map((lesson) => {
    ...
    return { ...unit, lessons: lessonWithCompetedStatus}
  });
...
export const getCourseProgress = cache(async () => {
  ...
  const firstUncompletedLesson = unitsInActiveCourse.flatMap((unit) => unit.lessons).find((lesson) => {
    return lesson.challenges.some((challenge) => {
      return !challenge.challengeProgress || challenge.challengeProgress.length === 0 || challenge.challengeProgress.some((progres) => progres.completed === false);
    });
  });
